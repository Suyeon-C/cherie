...
<body>
	<div class="text-center">
		<h3 class="pt-5 pb-4">구매자 회원 목록</h3>
		<%-- 검색 기능 --%>
		<form method="get" action="${contextPath}/admin/buyList" role="form">
		...
		<div class="container px-2 px-lg-5 mt-5">
			...
			<table class="table">
				...
				<c:forEach var="list" items="${sessionScope.lists}">
					<tbody>
						<tr>
							<td><a href="${contextPath}/buyer/update?buyEmail=${list.buyEmail}">
								${list.buyName}</a></td>
							<td>${list.buyEmail}</td>
							<td>${list.buyBirth}</td>
							<td>${list.buyPhone}</td>
							<td>${list.buyAddress}&nbsp; ${list.buyDaddress}</td>
							<td> <a href="${contextPath}/buyer/delete?buyEmail=${list.buyEmail}">
